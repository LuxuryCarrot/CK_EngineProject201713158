...
void SoftRenderer::Render2D()
{
	...
	size_t culledCircleCount = 0;
	size_t culledRectangleCount = 0;
	...
	for (auto it = _GameEngine.GoBegin(); it != _GameEngine.GoEnd(); ++it)
	{
		...
		goCircleBound.Radius *= gameObject.GetTransform().GetScale().X;
	...
	_RSI->PushStatisticText("Culled by Circle : " + std::to_string(culledCircleCount));
	_RSI->PushStatisticText("Culled by Rectangle : " + std::to_string(culledRectangleCount));
